 ...
 * Author: Anthony
 *
 * Created on September 27, 2016, 11:04 PM
 */
...
void DLinkedList<E>::swapFirstAndLast(){
    if(size() <2)
        throw underflow_error("List too short");
    DNode<E>* front = header->next;
    DNode<E>* last = trailer->prev;
    DNode<E>* frontNext = front->next;
    DNode<E>* lastPrev = last->prev;
    header->next = last;
    last->prev = header;
    last->next = frontNext;
    trailer->prev = front;
